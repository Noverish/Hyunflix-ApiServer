...
export function movieDetail(movie: Movie): MovieDetail {
  ...
  for (const file of files) {
    ...
    if (name === 'poster') {
      movieDetail.poster = 'http://' + join(FILE_SERVER, '/archive/Movies', movie.path, base);
    }

    if (name === 'thumbnail') {
      movieDetail.thumbnail = 'http://' + join(FILE_SERVER, '/archive/Movies', movie.path, base);
    }

    if (ext === '.smi' || ext === '.srt') {
      movieDetail.subtitles.push({
        language: name,
        src: 'http://' + join(FILE_SERVER, '/archive/Movies', movie.path, `${name}.vtt`),
      });
    }
    ...
    if (ext === '.mp4') {
      movieDetail.videos.push({
        ...
        src: 'http://' + join(FILE_SERVER, '/archive/Movies', movie.path, base),
      });
    }
  }
